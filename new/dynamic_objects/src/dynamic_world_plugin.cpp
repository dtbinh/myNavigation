...
public:
  ...
  void Load(physics::WorldPtr _world, sdf::ElementPtr _sdf) {
    ...
    if (_sdf->HasElement("z_increment_percentage"))
      this->z_increment_percentage =
          _sdf->Get<double>("z_increment_percentage");
    ...
    if (_sdf->HasElement("random_distribution_type")) {
      this->random_distribution_type =
          _sdf->Get<std::string>("random_distribution_type");
    } else {
    ...
    // yes if we want it to be active, no if we dont want elements of the scene
    // to change colours and lights
    if (_sdf->HasElement("colour_randomiser_active"))
      this->colour_randomiser_active =
          _sdf->Get<std::string>("colour_randomiser_active");
    ...
    if (_sdf->HasElement("model_to_ignore_randcolours"))
      ...
      models_to_ignore_colours_list =
          _sdf->Get<std::string>("model_to_ignore_randcolours");

    this->model_to_ignore_randcolours =
        this->split(models_to_ignore_colours_list, delimiter);

    for (auto &model_name_x : this->model_to_ignore_randcolours) {
      ROS_WARN("Ignore Colours model_name_x==%s", model_name_x.c_str());
    }

    if (this->check_if_vector_contains_string(this->model_to_ignore_randcolours,
                                              "rgb_cam")) {
    ...
    if (_sdf->HasElement("model_to_ignore_movement"))
      ...
      models_to_ignore_movement_list =
          _sdf->Get<std::string>("model_to_ignore_movement");

    this->model_to_ignore_movement =
        this->split(models_to_ignore_movement_list, delimiter);

    for (auto &model_name_x : this->model_to_ignore_movement) {
      ROS_WARN("Ignore MOV model_name_x==%s", model_name_x.c_str());
    }

    if (this->check_if_vector_contains_string(this->model_to_ignore_movement,
                                              "rgb_cam")) {
    ...
    if (!serviceName.empty())
      srv_ = this->rosNode->advertiseService(
          serviceName, &DynamicWorldPlugin::ServiceCallback, this);
    // start custom queue for imu
    callback_queue_thread_ = boost::thread(
        boost::bind(&DynamicWorldPlugin::CallbackQueueThread, this));

    ROS_DEBUG("Dynamic World Ready...The Only Thing That Is Constant Is Change "
              "- Quote by Heraclitus");
  }
  ...
  bool
  check_if_vector_contains_string(const std::vector<std::string> vector_obj,
                                  const std::string &s) {
    if (std::find(vector_obj.begin(), vector_obj.end(), s) !=
        vector_obj.end()) {
      return true;
    } else {
      return false;
  ...
  void Reset() {
    ...
    while (delta < 0.0) {
      // We change Direction
      ROS_INFO("Waiting until Clock is reseted and delta is not negative > "
               "Update delta=%f, new_secs=%f",
               delta, new_secs);
      new_secs = this->world->SimTime().Float();
      delta = new_secs - this->old_secs;
      ROS_INFO("Updated until Clock is reseted > Update delta=%f, new_secs=%f",
               delta, new_secs);
    }
...
public:
  ...
  void RandomiseWorldModels() {
    ...
    if (this->colour_randomiser_active == "yes") {
      ...
      ROS_WARN("The Randomiser is Disconected, no changes in the models will "
               "be made. Check the plugin input colour_randomiser_active");
    }
    ...
    for (auto const &x : this->modelIDToName) {
      ...
      if (!this->check_if_vector_contains_string(
              this->model_to_ignore_randcolours, model_name)) {
        result =
            this->ChangeModelColour(model_name, new_R, new_G, new_B, new_ALFA);
      }
    }
  }
  ...
  void UpdateWorldModels() {
    ...
    for (auto model : this->world->Models()) {
      ...
      for (auto link : model->GetLinks()) {
        // Get all the visuals, sdf is imported by Plugin.hh somehow , but not
        // the Link Definition
        ...
        if (linkSDF->HasElement("visual")) {
          for (sdf::ElementPtr visualSDF = linkSDF->GetElement("visual");
            ...
            ignition::math::Color ambient = materialSDF->GetElement("ambient")
                                                ->Get<ignition::math::Color>();
            ignition::math::Color diffuse = materialSDF->GetElement("diffuse")
                                                ->Get<ignition::math::Color>();
      ...
      if (numVecs == 0)
        numVecs = 1;
      ignition::math::Color color(r / numVecs, g / numVecs, b / numVecs,
                                  a / numVecs);
  ...
  bool ChangeModelColour(std::string model_name, int new_R, int new_G,
                         int new_B, int new_ALFA) {
    ...
    for (auto link : model->GetLinks()) {
      ...
      if (linkSDF->HasElement("visual")) {
        ...
        for (sdf::ElementPtr visualSDF = linkSDF->GetElement("visual");
          ...
          // std::string visual_element_string = link->GetScopedName() + "::" +
          // visualName; visMsg.set_name(visual_element_string);
  ...
  void MoveModels() {
    ...
    for (auto model : this->world->Models()) {
      ...
      // if (model_name != "rgb_cam" && model_name != "demo_table1" &&
      // model_name != "ground_plane" && model_name != "fetch" && model_name !=
      // "rubbish_bin1")
      if (!this->check_if_vector_contains_string(this->model_to_ignore_movement,
                                                 model_name)) {
        ...
        if (model_name == this->model_to_track_name) {
          ...
          z_pos_rand =
              RandomFloat(z_min_value, z_max_value) +
              models_counter * this->z_min * this->z_increment_percentage;
        }

        x_pos_rand = RandomFloat(x_min_value, x_max_value);

        // We generate y values random that follow a beta 0.5 0.5 distribution
        // to give more oportunities to have the object in the limits of the
        // table.
        if (this->random_distribution_type == "beta_distribution") {
          y_pos_rand = this->RandomFloatBeta(y_min_value, y_max_value);
        } else {
          ROS_WARN("The Distribution is Not BETA.==%s",
                   this->random_distribution_type.c_str());
        ...
        ROS_DEBUG(
            "POSE-RANDOM[X,Y,Z,Roll,Pitch,Yaw=[%f,%f,%f,%f,%f,%f], model=%s",
            x_pos_rand, y_pos_rand, z_pos_rand, roll_rand, pitch_rand, yaw_rand,
            model_name.c_str());
  ...
  void MoveCamera() {
    ...
    for (auto model : this->world->Models()) {
      ...
      if (model_name == "rgb_cam") {
        ...
        float pitch_rand =
            RandomFloat(this->cam_pitch_min, this->cam_pitch_max);
        float yaw_rand = RandomFloat(this->cam_yaw_min, this->cam_yaw_max);

        ROS_WARN(
            "CAM-RANDOM[X,Y,Z,Roll,Pitch,Yaw=[%f,%f,%f,%f,%f,%f], model=%s",
            x_pos_rand, y_pos_rand, z_pos_rand, roll_rand, pitch_rand, yaw_rand,
            model_name.c_str());
        ignition::math::Pose3d initPose(
            ignition::math::Vector3d(x_pos_rand, y_pos_rand, z_pos_rand),
  ...
  bool ChangeLight(std::string light_name) {
    ...
    float pitch_rand =
        RandomFloat(this->light_pitch_min, this->light_pitch_max);
    ...
    if (this->colour_randomiser_active == "yes") {
      gazebo::msgs::Set(
          msg.mutable_diffuse(),
          ignition::math::Color(diffuse_R, diffuse_G, diffuse_B, diffuse_A));
      gazebo::msgs::Set(
          msg.mutable_specular(),
          ignition::math::Color(specular_R, specular_G, specular_B, specular_A));
    } else {
      ROS_WARN("The Randomiser is Disconected, no changes in the lights will "
               "be made. Check the plugin input colour_randomiser_active");
    }

    gazebo::msgs::Set(msg.mutable_pose(),
                      ignition::math::Pose3d(x_pos_rand, y_pos_rand, z_pos_rand,
                                             roll_rand, pitch_rand, yaw_rand));
...
private:
  std::map<int, ignition::math::Color> lastKnownColors;
